...
def _cmd_status(raw_args: str) -> str:
    if not _is_caller_owner_via_env():
        return ""
    # v1.7.0: no-args → global PAID config summary (primary channel + identities)
    if not (raw_args or "").strip():
        owner = identity.load_owner()
        if owner is None:
            return "PAID: no owner.json yet — run `/paid-setup` first."
        enabled = [
            f"{i.platform}({i.user_id})"
            for i in owner.enabled_identities()
        ]
        primary = owner.preferred_platform or "(auto — first enabled)"
        pending_n = len(approval.list_pending())
        return (
            f"PAID config\n"
            f"  • 主频道: {primary}\n"
            f"  • Enabled identities: {', '.join(enabled) or '(none)'}\n"
            f"  • Pending approvals: {pending_n}\n"
            f"\n"
            f"Tip: `/paid-set-primary <platform>` to change primary channel.\n"
            f"     `/paid-status <id>` to inspect one request."
        )
    try:
        rid, _ = _resolve_request(raw_args)
    except ValueError as exc:
...
def _cmd_set_primary(raw_args: str) -> str:
    """``/paid-set-primary <platform>`` — set owner's primary IM channel.

    v1.7.0. Owner-only. Writes `OwnerProfile.preferred_platform` and
    re-derives owner.json so the runtime picks the new channel for all
    subsequent J3 approval cards, discovery cards, and alerts. Aliases:
    `feishu` → `lark`, `tg` → `telegram`. `auto` clears the field
    (PAID falls back to first enabled identity).
    """
    if not _is_caller_owner_via_env():
        return ""
    ...
    if not arg:
        return (
            "PAID: usage `/paid-set-primary <platform>` "
            "(lark / telegram / slack / auto)"
        )

    from paid import profile as _profile
    from paid import profile_sync as _profile_sync

    prof = _profile.load_profile()
    if prof is None:
        return (
            "PAID: no owner_profile.json yet — run `/paid-setup` first."
        )

    canonical = {"feishu": "lark", "lark": "lark",
                 "tg": "telegram", "telegram": "telegram",
                 "slack": "slack"}
    enabled = [
        str(i.get("platform", "") or "")
        for i in prof.identities
        if isinstance(i, dict) and i.get("enabled", True)
        and i.get("platform")
    ]
    enabled = [p for p in enabled if p]  # dedupe / drop blanks

    if arg in ("auto", "skip", "自动", ""):
        prof.preferred_platform = ""
        _profile.save_profile(prof)
        _profile_sync.derive_from_profile(prof)
        return (
            f"PAID: primary channel cleared — falling back to first "
            f"enabled identity ({enabled[0] if enabled else '(none)'})."
        )

    target = canonical.get(arg, arg)
    if target not in enabled:
        return (
            f"PAID: {arg!r} is not in your enabled identities "
            f"({', '.join(enabled) or '(none configured)'}). "
            f"Add it via `/paid-setup` first, or pick `auto` to clear."
        )

    prof.preferred_platform = target
    _profile.save_profile(prof)
    _profile_sync.derive_from_profile(prof)
    return f"PAID: primary channel set to {target}. New cards will route there."
...
def register(ctx) -> None:
    ...
    if cap["has_register_command"]:
        ...
        ctx.register_command(
            "paid-set-primary", _cmd_set_primary,
            description="Set owner's primary IM channel for PAID notifications (lark / telegram / slack / auto).",
            args_hint="<platform>",
        )
        ...
        _safe_log("commands: /paid-pending /paid-approve /paid-reject /paid-status /paid-cancel-input /paid-doctor /paid-set-primary /card")
